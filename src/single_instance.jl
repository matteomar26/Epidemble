...
#=function sibyl(N, T, Λ, O, γ, λ ; iters = 40, tol = 1e-14, learn=false,eta_learn=0.03)
    ...
    for st = 1:iters
        ...
        # if learn == flase && tolerance < tol ==> return m_sib,Infsteps. Come si trova tolerance??
        p_sib=[collect(n.bt) for n in f.nodes]
        if (learn == true && st>3)
